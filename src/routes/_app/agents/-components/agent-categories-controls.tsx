...
import { useAgentCategoryMutations } from "@/hooks/use-agent-categories";
import { cn } from "@/lib/utils";
import type { AgentCategory } from "@/types/agents";

// Paleta sóbria pras categorias novas; cicla pela quantidade já existente.
const AGENT_CATEGORY_PALETTE = ["#6366f1", "#0ea5e9", "#10b981", "#f59e0b", "#ef4444", "#a855f7"];

export function AgentCategoryCreateButton({ categories }: { categories: AgentCategory[] }) {
	const { create } = useAgentCategoryMutations();
	...
	function submit() {
		...
		if (!trimmed) return;
		const color = AGENT_CATEGORY_PALETTE[categories.length % AGENT_CATEGORY_PALETTE.length];
		create.mutate(
			{ name: trimmed, color },
			{
				onSuccess: () => {
...
export function AgentCategoryHeader({
	category,
	count,
	collapsed,
	onToggleCollapse,
}: AgentCategoryHeaderProps) {
	const { update, remove } = useAgentCategoryMutations();
		...
		<div className="group/header flex items-center gap-2 border-border/60 border-b pb-1">
			...
			{category && (
				<ConfirmDialog
					...
					description="Os agents dela voltam para “Sem categoria”."
