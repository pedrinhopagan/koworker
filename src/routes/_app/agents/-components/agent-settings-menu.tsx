...
// Menu único do agent (card): aparência e categoria são coisas separadas. "Aparência" abre
// o dialog; "Categoria" abre um submenu à direita que move o agent direto, sem dialog. No card ele
...
export function AgentSettingsMenu({
	...
	agent: { slug: string; categoryId: string | null };
	...
	function moveTo(categoryId: string | null) {
		if (categoryId === agent.categoryId) return;
		settingsMutation.mutate({ slug: agent.slug, categoryId });
	}
			...
			<DropdownMenuContent align="end" className="w-48">
					...
					<DropdownMenuSubContent className="max-h-72 w-52 overflow-y-auto">
						...
						{categories.map((category) => (
							<CategoryItem
								...
								active={agent.categoryId === category.id}
				...
				{docActions && (
					<>
						...
						<DropdownMenuItem onSelect={docActions.onCopyPath}>
							<Link2 />
							Copiar caminho
						</DropdownMenuItem>
						<DropdownMenuSeparator />
