...
export const Route = createLazyFileRoute("/_app/agents/$slug/")({
	component: AgentPage,
});
		...
		<div className="relative flex h-full w-full flex-col">
			{reading ? null : (
				<>
					...
					<DocMobileActionsDrawer open={actionsOpen} onClose={closeActions} title="Ações do agent">
						...
						{project && !isGlobal && !alreadyInProject && (
							<DocSheetActionButton
								icon={<FolderInput className="size-[18px]" />}
								label={`Injetar em ${project.name}`}
								disabled={injecting}
								onClick={() => {
									injectIntoProject();
									closeActions();
