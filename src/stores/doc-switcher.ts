...
// teclado (Alt+`) é estado local do próprio componente e não passa por aqui — este
...
interface DocSwitcherState {
	...
	open: () => void;
	close: () => void;
	setCurrent: (meta: CurrentSessionInput | null) => void;
}
...
export const useDocSwitcherStore = create<DocSwitcherState>((set) => ({
	...
	setCurrent: (meta) =>
		set({ current: meta ? { ...meta, lastVisited: Date.now(), pinned: false } : null }),
}));
